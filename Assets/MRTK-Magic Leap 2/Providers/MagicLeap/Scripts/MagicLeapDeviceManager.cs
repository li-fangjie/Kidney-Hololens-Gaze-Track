...
namespace MagicLeap.MRTK.DeviceManagement.Input
{
    ...
    public class MagicLeapDeviceManager : XRSDKDeviceManager
    {
        ...
        private MagicLeapOpenXRInputActionMap openXRInputs;
        private MagicLeapOpenXRInputActionMap.ControllerActions openXRControllerActions;

        private InputAction isTracked;
        ...
        private void SetupInput()
        {
            ...
            if (!mlControllerCallbacksActive)
            {
                ...
                {
                    openXRInputs = new MagicLeapOpenXRInputActionMap();
                    openXRInputs.Enable();

                    openXRControllerActions = new MagicLeapOpenXRInputActionMap.ControllerActions(openXRInputs);

                    isTracked = openXRControllerActions.IsTracked;
                }
        ...
        public override void Update()
        {
            if (IsEnabled && MLDevice.IsReady())
            {
                if (MLDevice.IsOpenXRLoaderActive())
                {
                    if (currentController == null && isTracked.IsPressed())
                    {
                        ConnectMLController();
                    }
                }
                ...
                if (MLDevice.IsOpenXRLoaderActive())
                {
                    didConnect = isTracked.IsPressed();
                }
        ...
        {
            if (MLDevice.IsMagicLeapLoaderActive() && mlControllerCallbacksActive)
            {
                mlInputs.Disable();
        ...
        {
            if (MLDevice.IsMagicLeapLoaderActive() && !controllerDevice.isValid)
            ...
            if (MLDevice.IsOpenXRLoaderActive() || (MLDevice.IsMagicLeapLoaderActive() && controllerDevice.isValid))
            {
                ...
                if (currentController == null)
                {
                    if (handedness != Handedness.None)
                    {
                        ...
                        MagicLeapMRTKController controller = new MagicLeapMRTKController(controllerDevice, TrackingState.NotTracked, handedness, inputSource, null, openXRControllerActions);
