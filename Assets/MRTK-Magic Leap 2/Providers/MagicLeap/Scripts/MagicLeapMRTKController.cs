...
namespace MagicLeap.MRTK.DeviceManagement.Input
{
    ...
    public class MagicLeapMRTKController : BaseController, IMixedRealityHapticFeedback
    {
        ...
        private MagicLeapOpenXRInputActionMap.ControllerActions openXRControllerActions;
        private Vector3 lastTouchVector;
        private float lastPressure;

        private InputAction triggerAction, triggerValueAction, bumperAction, menuButtonAction, positionAction, rotationAction,
            trackpadTouchAction, trackpadAction, trackpadForceAction, pointerPositionAction, pointerRotationAction;
        ...
        public MagicLeapMRTKController(InputDevice controller, TrackingState trackingState, Handedness controllerHandedness, IMixedRealityInputSource inputSource = null, MixedRealityInteractionMapping[] interactions = null, MagicLeapOpenXRInputActionMap.ControllerActions controllerMap = default) : base(trackingState, controllerHandedness, inputSource, interactions)
        {
            ...
            {
                openXRControllerActions = controllerMap;

                triggerAction = openXRControllerActions.Trigger;
                triggerValueAction = openXRControllerActions.TriggerValue;

                bumperAction = openXRControllerActions.Bumper;

                menuButtonAction = openXRControllerActions.MenuButton;

                positionAction = openXRControllerActions.Position;
                rotationAction = openXRControllerActions.Rotation;

                trackpadTouchAction = openXRControllerActions.TrackpadTouch;
                trackpadAction = openXRControllerActions.Trackpad;
                trackpadForceAction ??= openXRControllerActions.TrackpadForce;

                pointerPositionAction = openXRControllerActions.PointerPosition;
                pointerRotationAction = openXRControllerActions.PointerRotation;
            }
        }
        ...
        public void UpdatePoses()
        {
            ...
            if (MLDevice.IsMagicLeapLoaderActive())
            {
                isPositionAvailable = mlController.TryGetFeatureValue(UnityEngine.XR.CommonUsages.devicePosition, out Vector3 position);
                isRotationAvailable = mlController.TryGetFeatureValue(CommonUsages.deviceRotation, out Quaternion rotation);

                pointerPose = new MixedRealityPose(MixedRealityPlayspace.TransformPoint(position),
                    MixedRealityPlayspace.Rotation * rotation);
            }
            else
            {
                Vector3 openXRPosition = pointerPositionAction.ReadValue<Vector3>();
                isPositionAvailable = openXRPosition != Vector3.zero;

                Quaternion openXRRotation = pointerRotationAction.ReadValue<Quaternion>();
                isRotationAvailable = openXRRotation != Quaternion.identity;

                pointerPose = new MixedRealityPose(MixedRealityPlayspace.TransformPoint(openXRPosition),
                    MixedRealityPlayspace.Rotation * openXRRotation);
            }
            ...
            {
                if (trackpadTouchAction.IsPressed())
                {
                    HandleOnTouchpadDown(default, true);
                }
                else if (trackpadTouchAction.WasReleasedThisFrame())
                {
                    HandleOnTouchpadDown(default, false);
                }

                if (bumperAction.WasPressedThisFrame())
                {
                    MLControllerBumperDown(default);
                }
                else if (bumperAction.WasReleasedThisFrame())
                {
                    MLControllerBumperUp(default);
                }

                if (menuButtonAction.WasPressedThisFrame())
                {
                    MLControllerMenuDown(default);
                }
                else if (menuButtonAction.WasReleasedThisFrame())
                {
                    MLControllerMenuUp(default);
                }

                if (triggerAction.WasPressedThisFrame())
                {
                    MLControllerTriggerDown(default);
                }
                else if (triggerAction.WasReleasedThisFrame())
                {
                    MLControllerTriggerUp(default);
                }
            }
        }
        ...
        private void HandleOnTouchpadDown(UnityEngine.InputSystem.InputAction.CallbackContext obj, bool touchActive)
        {
            ...
            if (Interactions.Length > 4)
            {
                ...
                if (touchActive)
                {
                    if (MLDevice.IsOpenXRLoaderActive())
                    {
                        Interactions[5].Vector2Data = trackpadAction.ReadValue<Vector2>();
                        Interactions[6].FloatData = trackpadForceAction.ReadValue<float>();
                    }
                    else
                    {
