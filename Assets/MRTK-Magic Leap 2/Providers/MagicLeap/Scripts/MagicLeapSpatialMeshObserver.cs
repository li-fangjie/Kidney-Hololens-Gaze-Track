...
namespace MagicLeap.MRTK.SpatialAwareness
{
        ...
        BaseSpatialMeshObserver
    {
        ...
        private ISubsystem capiXRMeshingSubsystem;
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (capiXRMeshingSubsystem != null)
            {
                capiXRMeshingSubsystem.Stop();
                capiXRMeshingSubsystem.Destroy();
            }
        }
        ...
        public override void Update()
        {
            ...
            if(Profile != null && !permissionRequested)
            {
                ...
                Permissions.RequestPermissions(new string[] { Permissions.SpatialMapping }, OnPermissionGranted, OnPermissionDenied, OnPermissionDenied);
                return;
            }
        ...
        private void OnPermissionGranted(string permission)
        {
            if (permission == Permissions.SpatialMapping)
            {
                ...
                if (MLDevice.IsOpenXRLoaderActive() && !OpenXRSettings.Instance.GetFeature<MagicLeapMeshingFeature>().enabled)
                {
                    SubsystemStart();
                }
        ...
        private void SubsystemStart()
        {
            List<XRMeshSubsystemDescriptor> descriptors = new List<XRMeshSubsystemDescriptor>();

            SubsystemManager.GetSubsystemDescriptors<XRMeshSubsystemDescriptor>(descriptors);

            if (descriptors.Count > 0)
            {
                foreach (var descriptor in descriptors)
                {
                    if (String.Compare(descriptor.id, "MagicLeapXr-Meshing", true) == 0)
                    {
                        capiXRMeshingSubsystem = descriptor.Create();
                    }

                    if (capiXRMeshingSubsystem != null)
                    {
                        capiXRMeshingSubsystem.Start();
                        break;
                    }
                    else
                    {
                        Debug.LogError($"Failed to create Meshing Subsystem due to missing XRMeshingSubsystem.");
                    }
                }
            }
        }
