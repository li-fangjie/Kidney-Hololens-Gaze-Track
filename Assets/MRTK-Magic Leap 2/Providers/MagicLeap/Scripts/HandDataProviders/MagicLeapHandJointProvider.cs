...
namespace MagicLeap.MRTK.DeviceManagement.Input
{
    public class MagicLeapHandJointProvider 
    {
        ...
        private Microsoft.MixedReality.Toolkit.Utilities.Handedness _controllerHandedness;
        ...
        public MagicLeapHandJointProvider(Microsoft.MixedReality.Toolkit.Utilities.Handedness controllerHandedness)
        {
        ...
        #if XR_HANDS
        public void UpdateHandJoints(XRHand openXRHand)
        {
            if (JointPoses == null)
            {
                JointPoses = new Dictionary<TrackedHandJoint, MixedRealityPose>();
            ...
            for (int jointIndex = 1; jointIndex < (int)XRHandJointID.EndMarker; jointIndex++)
            {
                XRHandJoint currentJoint = openXRHand.GetJoint((XRHandJointID)jointIndex);
                bool foundJointPose = currentJoint.TryGetPose(out Pose subsystemJointPose);

                if (foundJointPose)
                {
                    MixedRealityPose mrtkPose = new MixedRealityPose();

                    mrtkPose.Position = MixedRealityPlayspace.TransformPoint(subsystemJointPose.position);
                    mrtkPose.Rotation = MixedRealityPlayspace.Rotation * subsystemJointPose.rotation;
                    
                    if (!JointPoses.ContainsKey((TrackedHandJoint)jointIndex))
                    {
                        JointPoses.Add((TrackedHandJoint)jointIndex, mrtkPose);
                    }
                    else
                    {
                        JointPoses[(TrackedHandJoint)jointIndex] = mrtkPose;
                    }
                }
            }
        }
        ...
        public void UpdateHandJoints(InvalidHands openXRHand)
        {
            return;
        }
