...
namespace MagicLeap.MRTK.DeviceManagement.Input
{
    ...
    public class MagicLeapHand : BaseHand
    {
        ...
        public MagicLeapOpenXRHandProcessor MagicLeapHandProcessor;

        private UnityEngine.XR.InputDevice  _hand;
        private UnityEngine.XR.InputDevice _gestureHand;
        ...
        private float gesturePinchValue = 0.03f;
        private float gesturePinchReleaseValue = 0.05f;

        private MagicLeapOpenXRInputActionMap.LeftHandActions lefthandActions;
        private MagicLeapOpenXRInputActionMap.RightHandActions rightHandActions;
        private InputAction graspValue;
        private InputAction pinchValue;

        public void Initalize(UnityEngine.XR.InputDevice hand)
        {
            _hand = hand;
            handRay = new HandRay();
        ...
        public void Initalize(MagicLeapOpenXRHandProcessor handProcessor)
        {
            MagicLeapHandProcessor = handProcessor;
            handRay = new HandRay();
            _magicLeapHandJointProvider = new MagicLeapHandJointProvider(ControllerHandedness);

            MagicLeapOpenXRInputActionMap openXRInputs = new MagicLeapOpenXRInputActionMap();
            openXRInputs.Enable();

            lefthandActions = new MagicLeapOpenXRInputActionMap.LeftHandActions(openXRInputs);
            rightHandActions = new MagicLeapOpenXRInputActionMap.RightHandActions(openXRInputs);

            if (ControllerHandedness == Handedness.Left)
            {
                graspValue = lefthandActions.GraspValue;
                pinchValue = lefthandActions.PinchValue;
            }
            else
            {
                graspValue = rightHandActions.GraspValue;
                pinchValue = rightHandActions.PinchValue;
            }
        }
        ...
        public void SetUseMLGestures(MagicLeapHandTrackingInputProfile.MLGestureType useMLGestures)
        {
            ...
            if (!gesturesStarted)
            {
                ...
                {
                    gesturesStarted = (ControllerHandedness == Handedness.Left) ? MagicLeapHandProcessor.LeftHand.isTracked : MagicLeapHandProcessor.RightHand.isTracked;
                }
            }
        }
            ...
            using (UpdateStatePerfMarker.Auto())
            {
                ...
                if (isTracked)
                {
                    ...
                    if (MLDevice.IsOpenXRLoaderActive())
                    {
                        if (ControllerHandedness == Handedness.Left)
                        {
                            _magicLeapHandJointProvider.UpdateHandJoints(MagicLeapHandProcessor.LeftHand);
                        }
                        else
                        {
                            _magicLeapHandJointProvider.UpdateHandJoints(MagicLeapHandProcessor.RightHand);
                        }
                    }
        ...
        private void CalculateGrab()
        {
            ...
            if (MLDevice.IsOpenXRLoaderActive())
            {
                if (useMLGestureClassification && gesturesStarted && graspValue.ReadValue<float>() > .8f)
                {
                    isGrabbingGesture = true;
                }
                else
        ...
        private void CalculatePinch()
        {
            ...
            if (MLDevice.IsOpenXRLoaderActive())
            {
                if (useMLGestureClassification && gesturesStarted && pinchValue.ReadValue<float>() > .8f)
                {
            ...
            // For interpreted pinch, compare the normalized pinch value with the profile settings for normalized trigger values.
            float pinchStrength = HandPoseUtils.CalculateIndexPinch(ControllerHandedness);
            // If we are already pinching, we make the pinch a bit sticky.  Otherwise, only consider pinching if finger confidence is high.
            bool isPinchingInterpreted = IsPinching ? pinchStrength > stickyPinch : pinchStrength > triggerPinch;
